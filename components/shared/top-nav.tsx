      ...
      <header className="sticky top-0 z-40 h-14 border-b border-border-default bg-bg-base/80 backdrop-blur-md">
        <div className="max-w-[1400px] mx-auto h-full px-6 flex items-center gap-8">
          <Link href="/" className="flex items-center gap-2 font-bold text-lg flex-shrink-0">
            <span className="w-6 h-6 rounded-md bg-gradient-to-br from-accent to-easy grid place-items-center text-bg-base font-extrabold text-[13px]">
              IO
            </span>
          ...
          <div className="flex items-center gap-3 flex-shrink-0 ml-auto">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-bg-surface border border-border-default rounded-md text-text-tertiary text-sm min-w-[160px] sm:min-w-[200px] hover:border-border-strong hover:bg-bg-elevated transition-colors"
              aria-label="Open search"
            >
              <Search className="w-4 h-4 flex-shrink-0" />
              <span className="hidden sm:inline flex-1 text-left">Search problems…</span>
        ...
        {mobileOpen && (
          <div id={mobileMenuId} className="md:hidden border-t border-border-default px-4 py-2">
            {LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
